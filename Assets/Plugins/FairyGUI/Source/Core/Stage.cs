...
#pragma warning restore 0649
		...
		public int[] GetAllTouch(int[] result)
		{
			...
			for (int j = 0; j < 5; j++)
			{
			    ...
			    if (Input.GetKey( "z" ) && j == touchCount - 1)
			    {
			        result[i++] = FguiTouchSimulator.simulateTouchPointId;
			        break;
			    }
#endif
				if (touch.touchId != -1)
        ...
        public void SortWorldSpacePanelsByDistance(int panelSortingOrder)
        {
            if (Camera.main == null)
            {
                return;
            ...
            for (int i = 0; i < numChildren; i++)
            {
                ...
                //借用一下tmpBounds
                obj._internal_bounds[0] = Vector3.Distance(Camera.main.transform.position,obj.cachedTransform.position);
